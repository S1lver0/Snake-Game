...
class juego{
    ...
    dibujar(direccion){
        let temporizador=setInterval(()=>{
            ...
            //chocar pared
            if(ultimo.x>600||ultimo.y>600||ultimo.x<0||ultimo.y<0){ // analiza si choco con pared
                this.derrota=true;
    ...
    dibujarComida(ctx,canvas,imagen){
        if(this.COMIDA.estado==false){
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            this.COMIDA.x= parseInt((Math.random()*(15-2)))*40; // se pone random del 2 al 20 y se multiplica x40 porque es lo que se mueve 
            this.COMIDA.y= parseInt((Math.random()*(15-2)))*40; // haciendo lo mismo para lograr proporcionalidad
    ...
    pantalla_perdio(temporizador){ // pantalla final 
        ...
        ctx.fillStyle="white";
        ctx.font = '50px Arial';
        ctx.fillText("JUEGO TERMINADO",70,90);
        ctx.drawImage(this.imagen,140,160,320,320);
        ctx.fillText("SCORE : ",140,550);
        ctx.fillText(this.COMIDA.contador,360,550);
        ctx.closePath();
    }
}
...
function IniciarJuego(){
    ...
    ctx.fillStyle="white";
    ctx.font = '35px Arial';
    ctx.fillText("CONTROLES :  W-A-S-D",110,200);
    ctx.font = '20px Arial';
    ctx.fillText("Presione una tecla para iniciar el juego",120,400);
    ctx.closePath();
